...
export function ProjectCard({ project }: { project: Project }) {
  ...
  ].filter((detail): detail is { label: string; body: string } =>
    Boolean(detail.body),
  );
  ...
  return (
    <>
      <Reveal className="col col-lg border-l border-line pl-8">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 border-y border-line py-3.5">
          {[
      ...
      <Reveal delay={0.1} className="col col-lg">
        ...
        {project.learned ? (
          <blockquote className="mb-4 border-l-2 border-signal pl-4">
        ...
        {project.note ? (
          <p className="flex gap-2.5 text-[0.6875rem] leading-relaxed text-ink-faint">
            <span
              aria-hidden
              className="mt-1 block h-px w-4 shrink-0 bg-line-bright"
            />
            {project.note}
          </p>
        ) : null}
